...
const BOOTH_CONTRAST_ALPHA = 1.3;
const BOOTH_BRIGHTNESS_BETA = 10;
const BOOTH_NOISE_STD = 15;
...
async function initHandLandmarker() {
  ...
  try {
    const handLandmarker = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Timed out downloading the HandLandmarker model (~10MB) on GPU."
    );
    return handLandmarker;
  } catch (gpuErr) {
    console.warn("[ArkanCam] GPU delegate failed, retrying on CPU…", gpuErr);
  }
  ...
  try {
    const handLandmarker = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Timed out downloading the HandLandmarker model (~10MB) even on CPU. Check your connection or whether storage.googleapis.com is blocked on your network."
    );
    return handLandmarker;
  } catch (cpuErr) {
    throw cpuErr;
...
function drawCountdownOverlay(box) {
  ...
  ctx.fillStyle = "rgba(0, 0, 0, 0.6)"; // Darkened for better contrast
  ctx.fillRect(box.x, box.y, box.width, box.height);

  ctx.font = `${Math.max(48, Math.min(box.width, box.height) * 0.4)}px 'Space Mono', monospace`;
...
function applyBoothEffect(imageData) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    let v = gray * BOOTH_CONTRAST_ALPHA + BOOTH_BRIGHTNESS_BETA;
    v += gaussianNoise(BOOTH_NOISE_STD);
...
function drawBoardAndPieces() {
  ...
  ctx.save();
  ctx.strokeStyle = "rgba(57, 255, 20, 0.2)"; // Dim green grid
  ctx.lineWidth = 1;
  ...
  for (const piece of sorted) {
    ctx.save();
    if (piece.dragging) {
      ctx.shadowColor = "rgba(57, 255, 20, 0.9)"; // Toxic green glow
      ctx.shadowBlur = 14;
    }
    ctx.drawImage(piece.canvas, piece.x, piece.y, piece.w, piece.h);
    ctx.strokeStyle = piece.placed ? "#39ff14" : "rgba(255, 255, 255, 0.3)";
  ...
  ctx.save();
  ctx.strokeStyle = puzzle.solved ? "#ffffff" : "#39ff14";
  ctx.lineWidth = 3;
  ctx.strokeRect(box.x, box.y, box.width, box.height);
  ctx.restore();
  ...
  if (puzzle.solved) {
    ...
    ctx.fillRect(box.x, box.y, box.width, box.height);
    ctx.font = `${Math.max(20, box.width * 0.07)}px 'Space Mono', monospace`;
    ctx.fillStyle = "#39ff14";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("SOLVED! — make a fist to save", box.x + box.width / 2, box.y + box.height / 2);
    ctx.restore();
  }
}``
...
function applyBWInsideBox(box) {
  ...
  applyBoothEffect(region);
  ctx.putImageData(region, x, y);
}

function drawLiveFrameOverlay(box) {
  ctx.save();
  ctx.strokeStyle = "#39ff14"; // Pure terminal green
  ctx.lineWidth = 3;
  ctx.strokeRect(box.x, box.y, box.width, box.height);
...
function finishShatter() {
  ...
  if (shatter.pendingCanvas) {
    ...
    statusText.textContent = "saved to your strip!";
    shatter.pendingCanvas = null;
  }
  resetPuzzleOnly();
}
  ...
  if (noHands) {
    ...
    if (appState === "puzzle") {
      ...
      statusText.textContent = puzzle.solved
        ? "puzzle solved! close your fist to save it"
        : "solve the puzzle with a pinch";
      return;
    }
...
async function boot() {
  ...
  const watchdog = setTimeout(() => {
    if (!settled) {
      showLoaderError("Loading is taking too long. Hit retry or check your connection.");
    }
  }, watchdogMs);
  ...
  try {
    ...
    if (err && err.name === "NotAllowedError") {
      showLoaderError("Camera permission denied. Enable it in your browser settings and hit retry.");
...
if (resetAllBtn) {
  resetAllBtn.addEventListener("click", () => {
    const confirmed = window.confirm(
      "Clear the whole photo strip and start over?"
    );
